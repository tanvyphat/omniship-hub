...
export function HistoryPage() {
  ...
  const createAction = features.outbound
    ? { label: 'Tạo phiếu xuất mới', path: `/warehouse/outbound/create?platform=${platform}` }
    : features.returns
      ? { label: 'Tạo phiếu hoàn mới', path: `/warehouse/returns/create?platform=${platform}` }
      : null;
  ...
  return (
    <div>
      <PageHeader
        eyebrow="History"
        title={`Lịch sử ${platform === 'tiktok' ? 'TikTok Shop' : 'Shopee'}`}
        description="Danh sách phiếu thuộc các module đang bật. Mỗi trang tối đa 100 phiếu."
        action={<Button variant="secondary" onClick={() => void load()} disabled={loading || featureLoading}><RefreshCw className="h-4 w-4" />Làm mới</Button>}
      />
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          <table className="w-full min-w-[980px] border-collapse text-left text-sm">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
                ...
                <th className="px-5 py-3 font-bold">Loại phiếu</th>
                <th className="px-5 py-3 font-bold">Số đơn</th>
                <th className="px-5 py-3 font-bold">Số dòng</th>
                <th className="px-5 py-3 text-right font-bold">Tổng SL</th>
                <th className="px-5 py-3 text-right font-bold">Thao tác</th>
              </tr>
            </thead>
            <tbody>
              {(loading || featureLoading) && <tr><td colSpan={7} className="px-5 py-16 text-center text-slate-500">Đang tải lịch sử…</td></tr>}
              {!loading && !featureLoading && error && <tr><td colSpan={7} className="px-5 py-16 text-center text-rose-600">{error}</td></tr>}
              {!loading && !featureLoading && !error && rows.length === 0 && <tr><td colSpan={7} className="px-5 py-16 text-center text-slate-500">Không có phiếu phù hợp với các module đang bật.</td></tr>}
              {!loading && !featureLoading && !error && rows.map((row, index) => (
                <tr key={row.id} className="border-b border-slate-100 hover:bg-slate-50">
                  <td className="px-5 py-3 text-slate-400">{page * PAGE_SIZE + index + 1}</td>
                  <td className="whitespace-nowrap px-5 py-3 font-medium text-slate-800">{formatDateVN(row.documentDate)}</td>
                  <td className="px-5 py-3">
                    <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${row.type === 'outbound' ? 'bg-blue-50 text-blue-700' : 'bg-amber-50 text-amber-700'}`}>
                      {row.type === 'outbound' ? 'Phiếu xuất hàng' : 'Phiếu hoàn hàng'}
                    </span>
                  </td>
                  <td className="px-5 py-3 font-semibold text-slate-800">{row.orderCount}</td>
        ...
        <div className="flex flex-col gap-3 border-t border-slate-100 p-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2">
            <Button variant="secondary" className="h-9 px-3" disabled={page === 0 || loading} onClick={() => setPage((current) => Math.max(0, current - 1))}><ChevronLeft className="h-4 w-4" />Trước</Button>
            <div className="rounded-lg bg-slate-100 px-3 py-2 text-xs font-bold text-slate-700">Trang {Math.min(page + 1, totalPages)} / {totalPages}</div>
            <Button variant="secondary" className="h-9 px-3" disabled={page >= totalPages - 1 || loading} onClick={() => setPage((current) => current + 1)}>Sau<ChevronRight className="h-4 w-4" /></Button>
          </div>
        </div>
      </section>
      ...
      {createAction && (
        <div className="mt-4 flex gap-2">
          <Button variant="ghost" onClick={() => navigate(createAction.path)}>{createAction.label}</Button>
        </div>
      )}
    </div>
  );
