...
export function DashboardPage() {
  ...
  const visibleCards = loading ? [] : actionCards.filter((card) => features[card.feature]);
  const actionGrid = visibleCards.length >= 3 ? 'lg:grid-cols-3' : visibleCards.length === 2 ? 'lg:grid-cols-2' : 'lg:grid-cols-1';
  const hasPlatformActions = !loading && (features.outbound || features.returns || features.history);
      ...
      <section>
        ...
        {loading ? (
          <div className="grid gap-4 lg:grid-cols-3">
            {[0, 1, 2].map((item) => <div key={item} className="h-52 animate-pulse rounded-[24px] border border-slate-200 bg-white/70" />)}
          </div>
        ) : visibleCards.length > 0 ? (
          <div className={`grid gap-4 ${actionGrid}`}>
            {visibleCards.map(({ title, description, icon: Icon, action, onClick, wrapper, glow }) => (
              <button
                key={title}
                type="button"
                onClick={onClick}
          ...
          <div className="rounded-[24px] border border-dashed border-slate-300 bg-white/70 p-10 text-center shadow-sm">
            <div className="text-base font-bold text-slate-700">Hiện chưa có module nào đang bật.</div>
            <div className="mt-1 text-sm text-slate-500">Administrator có thể bật lại chức năng từ Feature Control trên thanh phía trên.</div>
          </div>
        )}
      </section>
              ...
              <div className="relative flex items-start gap-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {features.outbound && <button type="button" onClick={() => navigate('/warehouse/outbound/create?platform=shopee')} className="cursor-pointer rounded-xl bg-orange-50 px-3 py-2 text-xs font-bold text-orange-700 transition hover:-translate-y-0.5 hover:bg-orange-100">Phiếu xuất</button>}
                    {features.returns && <button type="button" onClick={() => navigate('/warehouse/returns/create?platform=shopee')} className="cursor-pointer rounded-xl bg-slate-100 px-3 py-2 text-xs font-bold text-slate-700 transition hover:-translate-y-0.5 hover:bg-slate-200">Phiếu hoàn</button>}
                    {features.history && <button type="button" onClick={() => navigate('/warehouse/history/shopee')} className="cursor-pointer rounded-xl bg-slate-100 px-3 py-2 text-xs font-bold text-slate-700 transition hover:-translate-y-0.5 hover:bg-slate-200">Lịch sử</button>}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="group relative overflow-hidden rounded-[24px] border border-cyan-100 bg-white p-5 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-cyan-100/60 md:p-6">
              ...
              <div className="relative flex items-start gap-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {features.outbound && <button type="button" onClick={() => navigate('/warehouse/outbound/create?platform=tiktok')} className="cursor-pointer rounded-xl bg-cyan-50 px-3 py-2 text-xs font-bold text-cyan-700 transition hover:-translate-y-0.5 hover:bg-cyan-100">Phiếu xuất</button>}
                    {features.returns && <button type="button" onClick={() => navigate('/warehouse/returns/create?platform=tiktok')} className="cursor-pointer rounded-xl bg-slate-100 px-3 py-2 text-xs font-bold text-slate-700 transition hover:-translate-y-0.5 hover:bg-slate-200">Phiếu hoàn</button>}
                    {features.history && <button type="button" onClick={() => navigate('/warehouse/history/tiktok')} className="cursor-pointer rounded-xl bg-slate-100 px-3 py-2 text-xs font-bold text-slate-700 transition hover:-translate-y-0.5 hover:bg-slate-200">Lịch sử</button>}
                  </div>
                </div>
              </div>
            </div>
